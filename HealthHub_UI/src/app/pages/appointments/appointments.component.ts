...
this.updatePatientName();
   ...
   private updatePatientName(){
    ...
    this._http.get('http://localhost:8181/healthhubapi/getAllPatients')
    .map((res: Response) => res.json())
     .subscribe(data => {    this.patients = data;  
      for (let patient of this.patients) {
         
        this.patientmap.set(patient.patientId, ( patient.firstName + patient.lastName));
      }
   ...
   private updateDoctorName(){
    this._http.get('http://localhost:8181/healthhubapi/getAllPractitioners')
    .map((res: Response) => res.json())
     .subscribe(data => {    this.doctors = data;  
      
      for (let doctor of this.doctors) {
         
        this.doctormap.set(doctor.doctorId, ( doctor.firstName + doctor.lastName));
      }
   ...
   private loadIntialListOfAppointmentss() {
     this.isLoading = true;
     return this._http.get('http://localhost:8181/healthhubapi/listAllAppointmentss')
    .map((res: Response) => res.json())
      .subscribe(data => {
 ...
 public edit(id: string) {
  let navigationExtras: NavigationExtras = {
    ...
    this.router.navigate(['/Appointments/Appointmentsdetails'], navigationExtras);
  }   
  ...
  public delete(id:string) {
    ...
    this.isLoading = true;
    return this._http.delete('http://localhost:8181/healthhubapi/deleteAppointments/' + this.deleteId)    
     .subscribe(data => {            
