 ...
 isLoading: boolean = false;
  ...
  private createNewPatient() {
    ...
    if (this.editId  && this.editId !== ""  && this.editId !== "None") {
      ...
      return this._http.put('http://localhost:8181/healthhubapi/updatePatient', data22)
        .subscribe(
        res => {
          console.log(res);
      ...
      console.log(data22);
            return this._http.post('http://localhost:8181/healthhubapi/createPatient', data22)
        .subscribe(
        res => {
          console.log(res);
          this.isLoading = false;
...
ngAfterViewInit() {
  ...
  ngOnInit() {
    ...
    if (this.editId  && this.editId !== ""  && this.editId !== "None") {
      this.isLoading = true;
      return this._http.get('http://localhost:8181/healthhubapi/getPatientById/' + this.editId)
        .map((res: Response) => res.json())
        .subscribe(data => {
          var PatientList = data[0];
      

          this.doctorId = PatientList.doctorId; 
          this.firstName = PatientList.firstName;
          
          this.lastName = PatientList.lastName;
           
         
          this.email = PatientList.email;
